...
Class can have two types of attribute references: data or methods. Class methods are called
...
def test_method_objects():
    ...
    # object types can have methods as well. For example, list objects ahve methods called append,
    ...
    # But ba aware that counter.get_count() is not the same thing as MyCounter.get_counter() =
    ...
    # However, it is not necessary to call a method right away: counter.get)counter() is a method
    ...
    # Actually, you may have guessed the answer: the special thing about method is that the
    ...
    # calling a method with a list of n arguments is equivalent to calling the corrensponding
